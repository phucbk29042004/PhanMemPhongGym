...
import { checkinMethodLabel, formatDate, formatDateTime, unwrapData } from '../../utils/data';
...
export default function MemberProfileScreen() {
  ...
  const fetchData = async () => {
    try {
      ...
      setErrorText(error.response?.data?.message || 'Không tải được hồ sơ từ backend.');
    } finally {
      setLoading(false);
      setRefreshing(false);
    }
  ...
  const activePlan = profile?.goi_tap?.[0];
  const activePt = profile?.dang_ky_pt?.[0];
    ...
    >
      ...
      {profile ? (
        <View style={styles.profileCard}>
          ...
          <Text style={styles.meta}>{profile.so_dien_thoai || 'Chưa có số điện thoại'}</Text>
          <Text style={styles.meta}>{profile.email || 'Chưa có email'}</Text>
        </View>
      ) : null}

      <View style={styles.section}>
        <Text style={styles.sectionTitle}>Gói đang hoạt động</Text>
        <Text style={styles.rowText}>Gói tập: {activePlan?.ten_goi || 'Chưa có gói tập đang hoạt động'}</Text>
        <Text style={styles.rowText}>Hạn gói: {formatDate(activePlan?.den_ngay)}</Text>
        <Text style={styles.rowText}>PT: {activePt?.ten_pt || 'Chưa đăng ký PT'}</Text>
        <Text style={styles.rowText}>Buổi PT: {activePt ? `${activePt.so_buoi_da_tap}/${activePt.so_buoi_dang_ky}` : 'Chưa có'}</Text>
      </View>
...
const styles = StyleSheet.create({
  ...
  rowText: { color: '#3f4a3c', fontWeight: '600', marginBottom: 7 },
