...
import {
  ...
  KeyRound, LogOut, Moon, Phone, Shield, Sun,
  User, X,
...
function MenuRow({ icon: Icon, iconBg, iconColor, label, sublabel, onPress, rightEl, danger, colors }) {
  return (
    <TouchableOpacity
      style={[menuStyles.row, { borderBottomColor: danger ? (colors?.dangerLight || BRAND.dangerLight) : (colors?.borderLight || '#f0f4f0') }]}
      onPress={onPress}
      activeOpacity={0.7}
    >
...
const menuStyles = StyleSheet.create({
  row: { flexDirection: 'row', alignItems: 'center', paddingVertical: 12, paddingHorizontal: 4, gap: 12, borderBottomWidth: 1 },
...
const secStyles = StyleSheet.create({
  ...
  card: { borderRadius: 18, paddingHorizontal: 14, shadowColor: '#000', shadowOpacity: 0.04, shadowRadius: 10, elevation: 3 },
});
...
function ChangePasswordModal({ visible, onClose, colors }) {
  ...
  const reset = () => { setMatKhauCu(''); setMatKhauMoi(''); setXacNhan(''); setShowCu(false); setShowMoi(false); setShowXn(false); };
        ...
        <View style={[pwStyles.sheet, { backgroundColor: C.surface || '#ffffff' }]}>
          ...
          {[
            ...
            <View key={label} style={pwStyles.fieldWrap}>
              <Text style={[pwStyles.fieldLabel, { color: C.textSecondary || '#6b7c6b' }]}>{label}</Text>
              <View style={[pwStyles.inputRow, { backgroundColor: C.surfaceVariant || '#f0f4f0', borderColor: C.border || '#e4ebe4' }]}>
                <TextInput
                  style={[pwStyles.input, { color: C.text || '#141c14' }]}
                  value={val} onChangeText={set}
                  secureTextEntry={!show}
                  placeholder="••••••••" placeholderTextColor={C.textMuted || '#9cad9c'}
                  autoCapitalize="none"
                />
                <TouchableOpacity onPress={toggleShow} hitSlop={{ top: 8, bottom: 8, left: 8, right: 8 }}>
                  {show ? <EyeOff color={C.textMuted || '#9cad9c'} size={18} strokeWidth={2} /> : <Eye color={C.textMuted || '#9cad9c'} size={18} strokeWidth={2} />}
                </TouchableOpacity>
              </View>
            </View>
          ))}
          <View style={pwStyles.btnRow}>
            <TouchableOpacity style={[pwStyles.btnCancel, { borderColor: C.border || '#e4ebe4' }]} onPress={handleClose}>
              <Text style={[pwStyles.btnCancelText, { color: C.textSecondary || '#6b7c6b' }]}>Hủy</Text>
            </TouchableOpacity>
            <TouchableOpacity style={[pwStyles.btnSave, { opacity: saving ? 0.7 : 1 }]} onPress={handleSave} disabled={saving}>
...
const pwStyles = StyleSheet.create({
  ...
  btnSave: { flex: 1, height: 46, borderRadius: 12, backgroundColor: BRAND.primary, alignItems: 'center', justifyContent: 'center' },
    ...
    <View style={[styles.container, { backgroundColor: colors.background }]}>
      ...
      >
        ...
        <View style={styles.profileHeader}>
          ...
          {loading ? (
            <View style={styles.loadingBox}><ActivityIndicator size="large" color="#ffffff" /></View>
          ) : (
            <View style={[styles.profileCard, { backgroundColor: colors.surface }]}>
              ...
              <View style={[styles.roleBadge, { backgroundColor: BRAND.primaryLight }]}>
                <Shield color={BRAND.primary} size={12} strokeWidth={2.5} />
                <Text style={[styles.roleBadgeText, { color: BRAND.primary }]}>Quản trị viên</Text>
              </View>
              <View style={[styles.quickStats, { backgroundColor: colors.surfaceVariant }]}>
                <View style={styles.quickStat}>
                  <User color={colors.primary} size={16} strokeWidth={2} />
                  <Text style={[styles.quickStatVal, { color: colors.text }]}>{profile?.ten_dang_nhap || '—'}</Text>
                  <Text style={[styles.quickStatLabel, { color: colors.textMuted }]}>Tên đăng nhập</Text>
                </View>
                {profile?.so_dien_thoai && (
                  <>
                    ...
                    <View style={styles.quickStat}>
                      ...
                      <Text style={[styles.quickStatVal, { color: colors.text }]}>{profile.so_dien_thoai}</Text>
                ...
                {profile?.chi_nhanh && (
                  <>
                    ...
                    <View style={styles.quickStat}>
                      ...
                      <Text style={[styles.quickStatVal, { color: colors.text }]} numberOfLines={1}>{profile.chi_nhanh}</Text>
...
const styles = StyleSheet.create({
  ...
  avatarBadge: { position: 'absolute', bottom: 0, right: -2, width: 22, height: 22, borderRadius: 11, alignItems: 'center', justifyContent: 'center', borderWidth: 2 },
  profileName: { fontSize: 20, fontWeight: '800', textAlign: 'center', marginBottom: 8 },
  roleBadge: { flexDirection: 'row', alignItems: 'center', gap: 4, paddingHorizontal: 12, paddingVertical: 5, borderRadius: 20, marginBottom: 14 },
