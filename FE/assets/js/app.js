(function () {
  ...
  window.GymApp.showBirthdayEffect = function (callback) {
    ...
    const count = window.GymApp.data.members.filter(m => {
      const p = m.dob.split('-'); return `${p[1]}-${p[2]}` === todayMD;
    }).length;
  ...
  document.addEventListener('DOMContentLoaded', function () {
    ...
    document.addEventListener('click', function (e) {
      // Sidebar toggle
      if (e.target.closest('#sidebar-toggle')) { _toggleSidebar(); return; }
      // Nav items
      const navBtn = e.target.closest('[data-page]');
      if (navBtn && navBtn.dataset.page) { window.GymApp.navigate(navBtn.dataset.page); return; }
      // Accordion
      const accBtn = e.target.closest('[data-accordion]');
      if (accBtn) { _toggleAccordion(accBtn.dataset.accordion); return; }
      // Pagination
      const pgBtn = e.target.closest('[data-pg]');
      if (pgBtn && !pgBtn.disabled) {
        const pg = parseInt(pgBtn.dataset.pg);
        const handler = window.GymApp._pgHandler;
        if (handler && pg > 0) handler(pg);
      }
    });
