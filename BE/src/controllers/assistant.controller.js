...
- Luôn dùng tiếng Việt, xưng "mình" hoặc "Parry", gọi hội viên là "bạn". Nói chuyện tự nhiên như người thật, không cứng nhắc.`;
...
- Luôn dùng tiếng Việt, xưng "mình" hoặc "Parry", gọi PT là "thầy/cô" hoặc "bạn" tùy văn cảnh. Trò chuyện tự nhiên, thân thiện.`;
      ...
      const pendingApprovals = db.prepare(`SELECT COUNT(*) AS cnt FROM dang_ky_goi_tap WHERE trang_thai = 'cho_duyet'`).get()?.cnt || 0;
...
- Luôn dùng tiếng Việt, xưng "mình" hoặc "Parry". Thân thiện nhưng chuyên nghiệp — như một đồng nghiệp giỏi, không phải robot.`;
    }
