...
export const getRegistrations = (req, res) => {
  ...
  const rows = db.prepare(`
    SELECT
      ...
      dp.gia_thuc_te, dp.trang_thai, dp.ghi_chu, dp.ngay_tao,
...
export const createRegistration = (req, res) => {
  ...
  if (!hoi_vien_id || !pt_id || !so_buoi_dang_ky || !tu_ngay || gia_thuc_te === undefined) {
    return error(res, 'Thiếu: hoi_vien_id, pt_id, so_buoi_dang_ky, tu_ngay, gia_thuc_te', 400);
  }
  ...
  const result = db.prepare(`
    INSERT INTO dang_ky_pt
      (hoi_vien_id, pt_id, goi_pt_id, so_buoi_dang_ky, so_buoi_da_tap, tu_ngay, den_ngay, gia_thuc_te, ghi_chu, nguoi_tao_id)
    VALUES (?, ?, ?, ?, 0, ?, ?, ?, ?, ?)
  `).run(
    hoi_vien_id, pt_id, goi_pt_id || null,
    parseInt(so_buoi_dang_ky), tu_ngay, den_ngay || null,
    parseFloat(gia_thuc_te), ghi_chu || null, req.user.id,
  );
...
export const cancelRegistration = (req, res) => {
  ...
  db.prepare(`
    UPDATE dang_ky_pt SET trang_thai = 'da_huy', ghi_chu = COALESCE(?, ghi_chu) WHERE id = ?
